...
def front_matter(ctx: Any) -> List[Flowable]:
    ...
    abstract = (
        ...
        + (f". It also reverses the allocation ranking — the only place in "
           ...
           f"{_pretty_strategy(pen_au_winner).lower()}. Not because of the "
           f"taper, which this household is past before the test is "
           f"applied, but for the same reason: with no guaranteed income "
           f"underneath it, the portfolio's own worst outcomes become the "
           f"whole of retirement, and a smaller tail is worth more than a "
           f"larger mean"
...
def section_retirement(ctx: Any) -> List[Flowable]:
    ...
    out.append(ctx.p(
        ...
        "it — so working an extra year is costless here and produces "
        "income, and the certainty "
...
def section_longevity(ctx: Any) -> List[Flowable]:
    ...
    out.append(ctx.p(
        ...
        "between rules. A rule that sets its level from a planning horizon "
        "is handed the answer when the horizon is a known constant, because "
        "dividing by the years remaining is very nearly optimal once there "
        "is no uncertainty about how many there are. A rule that hedges "
        "longevity pays a premium against a risk the model has switched "
        "off. And ruin counts as failure a portfolio exhausted at "
        "ninety-one for an investor who most likely died some years "
        "before. What the fixed horizon flatters, then, is horizon-based "
        "rules as a class \u2014 and the table below shows it does not "
        "pick the same member of that class the survival-weighted "
        "objective does, which is a sharper way of putting the same point "
        "than naming one rule in advance would have been."))
    out.append(ctx.p(
...
def section_incidence(ctx: Any) -> List[Flowable]:
    ...
    if rule_arm is not None:
        out.append(ctx.p(
            ...
            + ("This is Proposition 1(ii) against 1(iii), and the sweep "
               "is what turns a signed derivative into a wanted equity "
               "share: the algebra says which side of the divide each "
               "rule falls on and cannot say how far, and the distance "
               "is the whole allocation."
               if "model" in _SECTION_NUMBER else
               "The budget line signs that difference and cannot size "
               "it \u2014 which side of the divide each rule falls on is "
               "algebra, and how far is this sweep. The derivation is "
               "Proposition 1 of " + (COMPANION["name"] if COMPANION
                                      else "the companion study") + ".")))
    out += ctx.figure(
        "fig66_incidence",
...
def section_limitations(ctx: Any) -> List[Flowable]:
    ...
    out.extend(ctx.bullets([x for x in [
        ...
        "<b>No disutility of labour in the baseline.</b> Working is free "
        "everywhere except Section #leisure, which charges for the years "
        "spent at it and turns the retirement date from a corner into an "
        "interior optimum. Everywhere else — including the timing results "
        "of Section #retirement, which for that reason require a matched "
        "comparison — the model cannot say when someone <i>should</i> "
        "retire, only what conditioning the date is worth. What is still "
        "missing is any value on the <i>kind</i> of work: a year is a year "
        "here, whoever spends it doing whatever.",
        "<b>A fixed horizon everywhere but two sections.</b> Death arrives "
        "at 93 with certainty in every result except Sections #mortality "
        "and #longevity. The first re-weights the headline under four "
        "Gompertz survival laws and finds the ranking unmoved, which is a "
        "narrower finding than it sounds: it re-weights a policy that does "
        "not itself respond to longevity, so a small answer is partly built "
        "in. The second closes that gap by re-solving the rule and the rate "
        "alongside the allocation under a survival curve, and finds the "
        "winner is a rule that never reads a mortality table at all. What "
        "is still missing is an annuity, which is where the value of "
        "longevity risk actually sits.",
...
def section_pension(ctx: Any) -> List[Flowable]:
    ...
    out.append(ctx.p(
        (f"<b>The point estimate reverses the ranking.</b> "
         ...
         f"countries can resolve that failure is a separate question, and "
         f"Section #ordering answers it differently for the two: the sign "
         f"holds in every sub-panel once the contribution rate is held "
         f"still, and in half of them for the system as legislated, which "
         f"is the arm reported here. The sign reported in this section is "
         f"therefore a point estimate, and every statement about it should "
         f"be read with that section's interval attached."
         if au_reorders else
    ...
    out.extend(ctx.bullets([
        ...
        (f"<b>The allocation ranking reverses \u2014 as a point estimate.</b> "
         f"It reverses in one other cell of Section #ordering's grid, and "
         f"that section shows the two are not equally well established: "
         f"the sign holds in every sub-panel once the contribution rate is "
         f"held still, and this cell \u2014 the system as legislated \u2014 "
